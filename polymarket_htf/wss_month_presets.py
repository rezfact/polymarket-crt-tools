...
default pullback / entry gates rarely fire (mostly ``timeout``). ``coarse_spot`` relaxes those gates
...
def apply_wss_month_preset(p: "WssMonthSimParams", preset: str) -> "WssMonthSimParams":
    ...
    - ``default``: unchanged.
    - ``coarse_spot``: smaller pullback requirement, shorter end buffer so coarse feeds (e.g. CRT 15m
      closes) can occasionally register ``paper_fill``. Does **not** replace proper 1m spot.
    - ``continuation``: aligns with :func:`polymarket_htf.wss_watch_presets.apply_sweet_spot_watch_preset`
      ``continuation`` (milder pullback, more entry minutes, looser Gamma cap) for local month runs
      vs ``watch_sweet_spot`` on VPS.
    - ``late_window``: :func:`polymarket_htf.crt_wss_monthly.late_fill_timing_ok` with
      ``late_fill_min_elapsed_sec=600`` (last ~5m of 15m window).
    - ``late_window_quality``: ``late_window`` plus ``max_retrace_frac=0.0009`` to avoid
      late stretched entries (research; tune by asset).
    """
    ...
    if name == "late_window":
        return replace(p, late_fill_min_elapsed_sec=600.0)
    if name == "late_window_quality":
        return replace(p, late_fill_min_elapsed_sec=600.0, max_retrace_frac=0.0009)
    raise ValueError(
        f"unknown WSS month preset {preset!r} (expected default, coarse_spot, continuation, late_window, late_window_quality)"
    )
